...
class PostSerializer(serializers.Serializer):
    ...
    def validate_video(self, value):
        ...
        if value.size > max_video_size:
            return serializers.ValidationError('Слишком большое видео')
        return value

    def validate_image(self, value):
        max_image_size = 5 * 1024 * 1024
        if value.size > max_image_size:
            return serializers.ValidationError('Слишком большое изображение')
        return value
    ...
    def validate_media_files(self, values):
        ...
        if len(values) > 30:
            return serializers.ValidationError('Слишком много медиафайлов')
        if 'video/mp4' in media_types_list and len(values) > 1:
            return serializers.ValidationError('Разрешено только 1 видео')
        for media_type in media_types_list:
            if media_type not in allowed_types:
                return serializers.ValidationError('Недопустимый тип медиа.')
